      ...
      if (!empty($desired_slug)) {
        ...
        if (!empty($subdir_name) && $subdir_name !== $desired_slug) {
          ...
          if (true === $GLOBALS["wp_filesystem"]->move($from_path, $to_path)) {
            ...
            return new WP_Error(
              "rename_failed",
              esc_html__("The remote plugin package does not contain a folder with the desired slug and renaming did not work.", "tgmpa") .
                " " .
                esc_html__("Please contact the plugin provider and ask them to package their plugin according to the WordPress guidelines.", "tgmpa"),
              [
                "found" => $subdir_name,
          ...
          return new WP_Error(
            "packaged_wrong",
            esc_html__("The remote plugin package consists of more than one file, but the files are not packaged in a folder.", "tgmpa") .
              " " .
              esc_html__("Please contact the plugin provider and ask them to package their plugin according to the WordPress guidelines.", "tgmpa"),
            [
              "found" => $subdir_name,
        ...
        class TGMPA_Bulk_Installer_Skin extends Bulk_Upgrader_Skin
        {
            ...
            if ("update" === $this->options["install_type"]) {
              ...
              if (version_compare($this->tgmpa->wp_version, "4.8", "<")) {
                $this->upgrader->strings["skin_update_successful"] .=
                  ' <a href="#" class="hide-if-no-js" onclick="%2$s"><span>' . esc_html__("Show Details", "tgmpa") . '</span><span class="hidden">' . esc_html__("Hide Details", "tgmpa") . "</span>.</a>";
              }
            }
          }
          ...
          public function bulk_footer()
          {
            ...
            if ($this->tgmpa->is_tgmpa_complete()) {
              ...
              echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
              $update_actions["dashboard"] = sprintf(esc_html($this->tgmpa->strings["complete"]), '<a href="' . esc_url(self_admin_url()) . '">' . esc_html($this->tgmpa->strings["dashboard"]) . "</a>");
            } else {
